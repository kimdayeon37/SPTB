pipeline {
    ...
    stages {
        stage('Make Jar') {
            steps {
                withCredentials([usernamePassword(credentialsId: 'angrymusic-sudo', passwordVariable: '1234', usernameVariable: 'angrymusic')]) {
                    script {
                        dir('./backend') {
                            sh './gradlew bootJar'
                            ...
                            sh "echo ${angrymusic} | sudo cp ${jarFilePath} /etc/init.d/sptb"

                            // Restart the service (modify the service name accordingly)
                            sh "echo ${angrymusic} | sudo service sptb restart"
                        }
                    }
                }   
            }
