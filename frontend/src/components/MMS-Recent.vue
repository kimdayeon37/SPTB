...
const openCard = (selectedData: SettingType) => {
  ...
  router.push({ name: 'MasterSerial', query: { selectedData: selectedDataJSON } })
}
...
const deleteCard = (index: number) => {
  ...
  const savedDataJSON = localStorage.getItem('masterSerialDataData')
  if (savedDataJSON) {
    const parsedData = JSON.parse(savedDataJSON) as SettingType[]
    parsedData.splice(index, 1)
    localStorage.setItem('masterSerialData', JSON.stringify(parsedData))
  }
}
...
const moveToTop = (index: number) => {
  if (buttonActivated.value && savedData.value) {
    ...
    localStorage.setItem('masterSerialData', JSON.stringify(savedData.value))
  }
}
</script>
    ...
    <div v-for="(data, i) in savedData" :key="i">
      <q-card class="my-card bg-secondary text-white">
        <q-card-section>
          ...
          <div class="text-subtitle2">[COM Port] {{ data.comPort }}</div>
          ...
          <div class="text-subtitle2">[TransactionDelay] {{ data.transactionDelay }}</div>
